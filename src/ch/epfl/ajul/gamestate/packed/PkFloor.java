...
/// Classe utilitaire permettant de manipuler le contenu d'une ligne plancher empaqueté dans une valeur
/// de type {@code int}.
///
...
public final class PkFloor {
    ...
    private static final int FLOOR_MASK = 0b111;
    ...
    public static int withAddedTiles(int pkFloor, int pkTileSet) {
        ...
        // Calcul des tuiles à ajouter lorsque la ligne plancher n'est pas pleine
        ...
        // Gestion du marqueur du premier joueur
        if (pkFloorSize == FLOOR_CAPACITY && PkTileSet.countOf(pkTileSet, TileKind.FIRST_PLAYER_MARKER) == 1) {
            pkFloorUpdated &= ~(FLOOR_MASK << (FLOOR_BITS * FLOOR_CAPACITY));
