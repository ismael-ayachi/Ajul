...
public final class PkPlayerStates {
    ...
    public static ImmutableIntArray initial(Game game) {
        ...
        for (PlayerId playerId : game.playerIds()) {
            int playerOffset = PLAYER_STATE_CONTENT_SIZE * playerId.ordinal();
            initialState[playerOffset + PATTERN_OFFSET] = PkPatterns.EMPTY;
            initialState[playerOffset + FLOOR_OFFSET] = PkFloor.EMPTY;
            initialState[playerOffset + WALL_OFFSET] = PkWall.EMPTY;
            initialState[playerOffset + POINTS_OFFSET] = 0;
        }
